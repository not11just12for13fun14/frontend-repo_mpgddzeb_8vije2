...
export default function HowItWorks() {
  ...
  return (
    <section id="how" className="relative py-20 md:py-28 bg-black">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center max-w-3xl mx-auto">
          <h2 className="text-3xl md:text-4xl font-extrabold tracking-tight text-white">How It Works</h2>
          <p className="mt-4 text-slate-300">Three simple steps to send better outreach.</p>
        </div>

        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {steps.map((s, i) => (
            <div key={i} className="rounded-2xl border border-slate-800 bg-gradient-to-br from-slate-950 to-black p-6 shadow-sm">
              <div className="h-11 w-11 rounded-xl bg-blue-600/15 text-blue-400 grid place-items-center">
                <s.icon className="h-5 w-5" />
              </div>
              <h3 className="mt-4 text-lg font-semibold text-white">Step {i + 1} — {s.title}</h3>
